...
const
//globális állandók
  SzerverMeghajto = 'W:';	//Ide kerülnek a gépadatok
  StationMeghajto = 'X:';	//Station pc-k  - gépadatok
  SzerverFelhasznalo = 'tabnt18\station';
  SzerverJelszo = 'Flextronics1';
  StationFelhasznalo = 'Administrator';
  StationJelszo = 'MS128';
...
begin
  ...
  begin
      dwType := RESOURCETYPE_DISK;
    	lpLocalName := PChar(SzerverMeghajto);
    	lpRemoteName := PChar(DirectoryPath);
  		lpProvider := nil;
  end;
	//ShowMessage(DirectoryPath);
  dwFlags := CONNECT_UPDATE_PROFILE;
  Result := WNetAddConnection2(Res, PChar(SzerverJelszo), PChar(SzerverFelhasznalo), dwFlags);
...
procedure Kapcsolat_vege();
begin
  WNetCancelConnection2(PChar(SzerverMeghajto),CONNECT_UPDATE_PROFILE,true);
end;
